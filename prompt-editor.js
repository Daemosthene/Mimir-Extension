...
function initializeEditor() {
  ...
  // Load initial prompt input text from storage
  chrome.storage.local.get('promptEditorInitialText', ({ promptEditorInitialText }) => {
    if (promptEditorInitialText) {
      promptInput.value = promptEditorInitialText;
      // If editor is empty, also set editor content to initial prompt
      if (!editor.innerText.trim()) {
        editor.innerText = promptEditorInitialText;
      }
    }
  });

  // Load edit text content from storage and set editor content only if present
  chrome.storage.local.get('promptEditorEditText', ({ promptEditorEditText }) => {
    if (promptEditorEditText) {
      editor.innerText = promptEditorEditText;
    } else {
      // Clear editor if no edit text stored
      editor.innerText = "";
    }
  });
