  ...
  function runToolInSidebar(action, text) {
    ...
    fetch("https://mimir-server-daemosthene-mimir-extension.vercel.app/api/mimir", {
      ...
      .catch(err => {
        ...
        // If CORS error, network issue, or non-JSON response, try the proxy
        if (useProxyFallback || err.message.includes('CORS') || err.message.includes('NetworkError') || err.message.includes('HTML instead of JSON')) {
          ...
          .catch(proxyErr => {
            ...
            let errorMessage = `Error: ${proxyErr.message || "Failed to connect to API server."}`;
            if (proxyErr.message.includes("HTML instead of JSON")) {
              errorMessage = "Server error: API is returning HTML instead of JSON. Please try again later.";
            }
